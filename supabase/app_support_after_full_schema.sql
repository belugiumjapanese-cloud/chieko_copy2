...
-- Avoid recursive community/member RLS reads from the first schema draft.
-- Policy subqueries that read communities and community_members can recurse.
-- These security definer helpers centralize those checks without re-entering
-- the caller's row policies.
...
create policy "community_members leave self"
on public.community_members
for delete
using (
  user_id = auth.uid()
...
create policy "community_posts readable"
...
create policy "community_posts insert own post as member"
on public.community_posts
for insert
with check (
  user_id = auth.uid()
...
create policy "community_posts delete own or owner"
on public.community_posts
for delete
using (
  user_id = auth.uid()
...
create policy "community_messages readable"
...
create policy "community_messages insert member"
on public.community_messages
for insert
with check (
  user_id = auth.uid()
